...
# These come from the compose.yml file's "environment" section
DB_HOST = os.getenv("DB_HOST", "db")  # "db" is the service name in compose.yml
DB_PORT = int(os.getenv("DB_PORT", "5432"))  # Default PostgreSQL port
DB_USER = os.getenv("DB_USER", "appuser")  # Must match POSTGRES_USER in compose.yml
DB_PASS = os.getenv("DB_PASS", "secretpw")  # Must match POSTGRES_PASSWORD in compose.yml
DB_NAME = os.getenv("DB_NAME", "appdb")  # Must match POSTGRES_DB in compose.yml
